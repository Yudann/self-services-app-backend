...
public class TransactionController {
    ...
    @GetMapping("/last")
    public ResponseEntity<ApiResponse<Transactions>> getLastTransaction() {
    Transactions lastTransaction = transactionsRepository.findTopByOrderByCreatedAtDesc();

    if (lastTransaction == null) {
        return ResponseEntity.status(404).body(
            new ApiResponse<>(404, "No transactions found", null)
        );
    }

    return ResponseEntity.ok(
        new ApiResponse<>(200, "Success", lastTransaction)
    );
}
